...
from Compare_config.arguments_morphFormer import get_parser
...
if __name__ == '__main__':
    ...
    for exp_idx,curr_seed in enumerate(seed_list):
        ...
        # build Model
        # net = MamTrans(channels = channels, num_classes= class_count,image_size=value,datasetname=data_set_name)
        net = get_model("morphFormer","ip",value)
        net.to(device)
        ...
        if record_computecost:
            net.eval()  # 将模型设置为评估模式
            model = net
            input_shape = (1,channels, value, value)  # 输入形状
        ...
        # best_net = MamTrans(channels=channels, num_classes=class_count, image_size=value, datasetname=data_set_name)
        best_net = get_model("morphFormer","ip",value)
        best_net.to(device)
