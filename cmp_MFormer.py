...
from Compare_config.arguments_Former import get_parser
...
from Com_MFormer.videomamba import VisionMamba
...
if __name__ == '__main__':
    ...
    for exp_idx,curr_seed in enumerate(seed_list):
        ...
        #     k_group=4,
        #     embed_dim=32,
        #     dt_rank=math.ceil(2),  # Rank of the dynamic routing tensor
        #     d_inner=64,  # 256,  # Inner dimension of the model
        #     d_state=16,  # 256,  # State dimension of the model
        #     num_classes=class_count,  # Number of output classes pu 9 ip 16 houston 15
        #     depth=1,
        ...
        # )
        net = VisionMamba(
            model_type='Parallel MT',
            embed_dim=32,
            d_state=16,  # State dimension
            ssm_ratio=1,
            num_classes=15,  # 输出类别数
            ...
            # dim_linear_i = pca_components - conv3D_kernel_i[0] + 1
            # dim_linear_1 = 30 - 5 + 1 = 26
            # dim_linear_2 = 30 - 7 + 1 = 24
            # dim_linear_3 = 30 - 9 + 1 = 22
            dim_linear_1=26,
            dim_linear_2=24,
            dim_linear_3=22,
        )
        ...
        if record_computecost:
            net.eval()  # 将模型设置为评估模式
            model = net
            input_shape = (1, 1, channels, value, value)  # 输入形状
        ...
        for epoch in range(max_epoch):
            ...
            for X_train,y_train in train_iter:
                ...
                X_train = X_train.permute(0, 4, 3, 1, 2)  # -> (64, 1, 200, 11, 11)# 在最后一维增加一个维度
            ...
            with torch.no_grad():
                ...
                for X_val, y_val in val_iter:
                    ...
                    X_val = X_val.permute(0, 4, 3, 1, 2)
                    X_val = X_val.to(device)
                    y_val = y_val.to(device)
                    y_val_pred, _ = net(X_val)
            ...
            if (epoch + 1) % 50 == 0:
                ...
                vis_a_image(gt, full_predict, save_single_predict_path, save_single_gt_path)
        ...
        with torch.no_grad():
            ...
            for X_test, y_test in test_iter:
                ...
                X_test = X_test.permute(0, 4, 3, 1, 2)
                X_test = X_test.to(device)
                y_test = y_test.to(device)
                y_test_pred, _ = best_net(X_test)
