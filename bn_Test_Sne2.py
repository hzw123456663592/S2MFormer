...
#     labels = np.array(labels).astype(int)  # ⭐ 关键一行
#     tsne = TSNE(
#         n_components=2,
#         perplexity=30,
#         init='pca',
...
if __name__ == '__main__':
    ...
    for exp_idx,curr_seed in enumerate(seed_list):
        ...
        # ===================== 原始特征 t-SNE（只画一次） =====================
        if exp_idx == 0:
            logger.info("Generating raw feature t-SNE (only once)")
            all_raw_features = []
            all_raw_labels = []
            ...
            for idx in all_data_index:
                ...
                all_raw_labels.append(gt[i, j] - 1)  # 0..15，与训练/测试一致
