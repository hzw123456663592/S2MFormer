...
class SpaMamba(nn.Module):
    ...
    def forward(self, x):
        ...
        # 2. 从下到上扫描
        x_bottom_to_top = torch.flip(x, dims=[2]).permute(0, 2, 3, 1).contiguous()  # B, H, W, C
        x_bottom_to_top = x_bottom_to_top.view(B, H * W, C)
        x_bottom_to_top = self.mamba(x_bottom_to_top)
        x_bottom_to_top = x_bottom_to_top.view(B, H, W, C).permute(0, 3, 1, 2).contiguous()
        ...
        # 4. 从右到左扫描
        x_right_to_left = torch.flip(x, dims=[3]).permute(0, 3, 2, 1).contiguous()  # B, W, H, C
        x_right_to_left = x_right_to_left.view(B, W * H, C)
        x_right_to_left = self.mamba(x_right_to_left)
        x_right_to_left = x_right_to_left.view(B, W, H, C).permute(0, 3, 2, 1).contiguous()
