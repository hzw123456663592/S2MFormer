...
class MambaHSI(nn.Module):
    def __init__(self, in_channels=128,
        ...
        if mamba_type == 'spa':
            ...
            # 为每组特征添加可学习权重（例如3组：spa_x1, spa_x2, spa_x3）
        self.weight_spa = nn.Parameter(torch.ones(3))  # 初始化为均等权重
        self.weight_spe = nn.Parameter(torch.ones(3))  # 光谱分支的权重（如果存在）
    ...
    def forward(self, x):
        ...
        if self.mamba_type == 'spa' and self.trans:
            ...
            # 获取权重并softmax归一化
            weights_spa = F.softmax(self.weight_spa, dim=0)
            # 加权拼接
            # LG = torch.cat([
            #     weights_spa[0] * spa_x1,
            #     weights_spa[1] * spa_x2,
            #     weights_spa[2] * spa_x3
            # ], dim=1)
        ...
        if self.mamba_type == 'spe' and self.trans:
            ...
            # 获取权重并softmax归一化
            weights_spe = F.softmax(self.weight_spe, dim=0)

            # 加权拼接
            # LG = torch.cat([
            #     weights_spe[0] * spe_x1,
            #     weights_spe[1] * spe_x2,
            #     weights_spe[2] * spe_x3
            # ], dim=1)
